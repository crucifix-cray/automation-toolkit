...
- **Mega/Local:** `72 lovable local` (`scripts/sessions/session-*` 72 dirs, `finals/lovables.json` 11 total `9 verified` incl `genev.aochea` + `samant.habaloradan` `2FA`) + `36 lovable_sessions` on `mega:chimera/lovable_sessions` (partial sync) + `69 chimera (62 active)` + `105 railway_sessions (7 reindexed)` via `LD_PRELOAD="" rclone --mega-use-https` raw IP + `eu.api.mega.co.nz` fix. 1 bridge needed. **AI note:** `72` = `scripts/sessions/` `60+` you remember, `11` = `finals/lovables.json` subset, `62` = `chimera` miner not lovable.
