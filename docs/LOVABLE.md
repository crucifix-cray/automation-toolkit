...
- Script 3 (mine): `chimera-miner` `daemon.py` + `miner_injector.py` (Railway cells; navigate Preview → `/term` then **`doc('nproc')`** — URL alone ≠ bridge)

**Notes (2026-09-24):** Cells **13 / 16 / 28 / 35** mining forever after `doc('nproc')` gate. Script 2 prompt ends keep-as-is / no clarifying questions. cell-16 locked — do not reuse Railway `sessions/session-2`. OnKernel: org ≤5 concurrent; unlocked farm keys. Remaining cells need bridge or auth fix.
