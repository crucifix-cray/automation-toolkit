...
async def sign_in_to_railway(page, mailbox: MailTmInbox):
    ...
    print("⏳ Waiting for Turnstile to pass (up to 5 minutes)...")
    
    deadline = asyncio.get_running_loop().time() + 300  # 5 minutes timeout
    attempt = 0
    while asyncio.get_running_loop().time() < deadline:
        try:
            attempt += 1
            if attempt % 10 == 0:
                print(f"  Still waiting for Turnstile... ({attempt}0s elapsed)")
            
            if await continue_btn.is_enabled(timeout=1000):
                print("✅ Turnstile passed!")
                await page.wait_for_timeout(2000)  # Extra wait before clicking
        ...
        raise RuntimeError(f"Turnstile timeout after 5 minutes - check {screenshot_path}")
