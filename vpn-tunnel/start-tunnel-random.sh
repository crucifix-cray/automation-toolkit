#!/bin/bash
# Random WARP Tunnel - picks random ovpn from protonvpn-pool + WARP + Chrome
TUNNEL_DIR="/home/alae/.vpn-tunnel"
POOL_DIR="$TUNNEL_DIR/protonvpn-pool"
NS_NAME="vpnns"
...
# Pick random ovpn
if [[ ! -d "$POOL_DIR" ]]; then echo "[-] Pool not found: $POOL_DIR"; exit 1; fi
OVPN_FILE=$(ls "$POOL_DIR"/*.ovpn 2>/dev/null | shuf -n 1)
if [[ -z "$OVPN_FILE" ]]; then echo "[-] No ovpn files in $POOL_DIR"; exit 1; fi
OVPN_NAME=$(basename "$OVPN_FILE")
REMOTE_IP=$(grep -m1 "^remote " "$OVPN_FILE" | awk '{print $2}')
REMOTE_PORT=$(grep -m1 "^remote " "$OVPN_FILE" | awk '{print $3}')
[[ -z "$REMOTE_PORT" ]] && REMOTE_PORT=1194
# save for cleanup
echo "$REMOTE_IP" > /tmp/chosen-ovpn-ip
echo "$OVPN_NAME ($REMOTE_IP:$REMOTE_PORT)" > /tmp/chosen-ovpn-name
...
cleanup() {
    trap - EXIT INT TERM
    CHOSEN_IP=$(cat /tmp/chosen-ovpn-ip 2>/dev/null || echo "149.40.62.31")
    log "Shutting down..."
    ...
    ip route del "$CHOSEN_IP/32" via 192.168.100.1 dev wlan0 2>/dev/null || true
...
log "Random WARP Tunnel - chosen: $OVPN_NAME ($REMOTE_IP:$REMOTE_PORT) [$(ls $POOL_DIR/*.ovpn | wc -l) in pool]"
...
iptables -t nat -C POSTROUTING -s 10.200.0.0/30 -o "$WLAN_IFACE" -j MASQUERADE 2>/dev/null || iptables -t nat -A POSTROUTING -s 10.200.0.0/30 -o "$WLAN_IFACE" -j MASQUERADE
...
iptables -C FORWARD -i veth-host -o "$WLAN_IFACE" -j ACCEPT 2>/dev/null || iptables -A FORWARD -i veth-host -o "$WLAN_IFACE" -j ACCEPT
iptables -C FORWARD -i "$WLAN_IFACE" -o veth-host -m state --state RELATED,ESTABLISHED -j ACCEPT 2>/dev/null || iptables -A FORWARD -i "$WLAN_IFACE" -o veth-host -m state --state RELATED,ESTABLISHED -j ACCEPT
...
ip netns exec "$NS_NAME" ip link del warp-wg 2>/dev/null || true
ip netns exec "$NS_NAME" ip link del tun1 2>/dev/null || true
ip route add "$REMOTE_IP/32" via 192.168.100.1 dev wlan0 2>/dev/null || true
...
ip netns exec "$NS_NAME" ip route add "$REMOTE_IP/32" via 10.200.0.1 dev veth-ns onlink 2>/dev/null || true
ip netns exec "$NS_NAME" ip route add default via 10.200.0.1 dev veth-ns 2>/dev/null
sleep 1

# Block WebRTC
ip netns exec "$NS_NAME" iptables -A OUTPUT -p udp --dport "$REMOTE_PORT" -d "$REMOTE_IP" -j ACCEPT 2>/dev/null || true
...
# Generate ovpn-fixed.conf from chosen pool file (always regenerate for random)
log "Generating OpenVPN config from $OVPN_NAME..."
...
auth-user-pass $TUNNEL_DIR/credentials.txt
disable-dco
pull-filter ignore "redirect-gateway"
pull-filter ignore "dhcp-option"
verb 4
OVPN
sed -n '/<ca>/,/<\/ca>/p' "$OVPN_FILE" >> /tmp/ovpn-fixed.conf
sed -n '/<tls-crypt>/,/<\/tls-crypt>/p' "$OVPN_FILE" >> /tmp/ovpn-fixed.conf
chmod 600 /tmp/ovpn-fixed.conf

log "Starting OpenVPN ($OVPN_NAME $REMOTE_IP)..."
...
log "OpenVPN IP: $OVPN_IP ($OVPN_NAME)"
...
ip netns exec "$NS_NAME" wg set warp-wg private-key "$TUNNEL_DIR/warp.key" peer bmXOC+F1FxEMF9dyiK2H5/1SUtzH0JuVo51h2wPfgyo= endpoint 162.159.192.1:2408 allowed-ips 0.0.0.0/1,128.0.0.0/1,::/1,128.0.0.0/2
...
CHAIN_LABEL="Random $OVPN_NAME -> WARP"
PROFILE_DIR="$TUNNEL_DIR/chrome-profile-random"
if [[ -n "$OVPN_ONLY" ]]; then
    CHAIN_LABEL="Random $OVPN_NAME only"
    PROFILE_DIR="$TUNNEL_DIR/chrome-profile-random-ovpn"
fi
...
ip netns exec "$NS_NAME" sudo -u alae env DISPLAY=:0 XDG_RUNTIME_DIR=/run/user/1000 WAYLAND_DISPLAY=wayland-1 /opt/google/chrome/chrome --user-data-dir="$PROFILE_DIR" $OZONE_FLAG "https://browserleaks.com/ip" >/tmp/chrome-random.log 2>&1 &
disown

log "============================================"
log "  $CHAIN_LABEL -> Chrome"
log "  Chosen: $OVPN_NAME $REMOTE_IP"
