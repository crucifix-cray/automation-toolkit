#!/usr/bin/env python3
"""
Railway CLI Account Creator - Dispose.lol Edition
Complete implementation with OAuth, ToS acceptance, and CLI session registration
Uses dispose.lol Gmail instead of mail.tm
"""
...
class DisposeLolInbox:
    ...
    def __init__(self, context=None):
        self.context = context  # Browser context to create new page
        self.page = None  # Separate page for dispose.lol
        self.address = None
    ...
    async def _ensure_page(self):
        ...
        if not self.session_initialized:
            # Create NEW page (tab) for dispose.lol
            self.page = await self.context.new_page()
            await self.page.goto(self.BASE_URL, wait_until="load", timeout=60000)
            await self.page.wait_for_timeout(3000)
            
            # Check for Cloudflare Turnstile on dispose.lol
            print("  🔍 Checking for dispose.lol Cloudflare protection...")
            try:
                # Look for Cloudflare iframe
                cf_iframe = self.page.locator('iframe[src*="challenges.cloudflare.com"]')
                if await cf_iframe.count() > 0:
                    print("  ⚠️  Dispose.lol has Cloudflare protection, waiting for it to pass...")
                    
                    # Try to solve it with playwright-captcha if available
                    if CAPTCHA_SOLVER_AVAILABLE:
                        try:
                            from playwright_captcha import CaptchaType, ClickSolver
                            solver = ClickSolver(page=self.page)
                            await solver.solve_captcha(CaptchaType.CLOUDFLARE_TURNSTILE)
                            print("  ✅ Cloudflare challenge solved")
                        except Exception as e:
                            print(f"  ⚠️  Auto-solve failed: {e}, waiting manually...")
                    
                    # Wait for Cloudflare to pass (up to 30 seconds)
                    for i in range(30):
                        await self.page.wait_for_timeout(1000)
                        # Check if we can now see the email
                        has_email = await self.page.evaluate('''() => {
                            return document.body.textContent.includes('@gmail.com');
                        }''')
                        if has_email:
                            print("  ✅ Cloudflare passed, page loaded")
                            break
                    else:
    ...
    async def create(self):
        ...
        # Wait for page to load and email to appear
        await self.page.wait_for_timeout(5000)
        
        # The email is in a text element near the "Copy" button
        # Look for text containing @gmail.com
        try:
            # Try to find the email text directly
            email_text = await self.page.evaluate('''() => {
                // Find all text nodes containing @gmail.com
        ...
        # Fallback: take screenshot and raise error
        await self.page.screenshot(path="/tmp/disposelol-error.png", full_page=True)
        raise Exception("Could not find dispose.lol email address - check /tmp/disposelol-error.png")
    ...
    async def wait_for_railway_code(self, timeout_seconds=300):
        ...
        # Already on dispose.lol page in separate tab, just refresh
        while time.time() < deadline:
            check_count += 1
            
            # Refresh the dispose.lol inbox page
            ...
            for button in message_buttons:
                ...
                if aria_label and 'railway' in aria_label.lower():
                    ...
                    if match:
                        ...
                        # No need to navigate back - Railway is in separate tab
                        return otp
...
async def dismiss_cookie_banner(page):
    ...
    try:
        osano = railway_page.locator('[role="dialog"][aria-label*="Cookie"]').first
        if await osano.count() > 0 and await osano.is_visible():
            ...
            if await reject_btn.count() > 0:
                ...
                await railway_page.wait_for_timeout(500)
    except:
        pass


async def sign_in_to_railway(railway_page, mailbox: DisposeLolInbox):
    """Sign in to Railway with email (railway_page is separate from dispose.lol page)"""
    ...
    await railway_page.wait_for_timeout(2000)  # Wait for Turnstile to load
    ...
    if not otp_filled:
        try:
            ...
            await railway_page.wait_for_selector('iframe[src*="auth.magic.link"], iframe[src*="magic"]', timeout=10000)
            magic_frame = railway_page.frame_locator('iframe[src*="auth.magic.link"], iframe[src*="magic"]')
    ...
    if not otp_filled:
        try:
            inputs = railway_page.locator('input[inputmode="numeric"]:visible, input[inputmode="text"]:visible')
    ...
    if not otp_filled:
        ...
        await railway_page.screenshot(path="/tmp/railway_otp_failure.png")
...
async def accept_railway_policies(railway_page):
    ...
    await railway_page.wait_for_timeout(2000)
    await dismiss_cookie_banner(railway_page)
...
async def register_cli_session(context, railway_page, railway_dir: Path) -> Path:
        ...
        if use_warp:
            ...
            if rotate_warp_ip():
                ...
                if not warp_started:
                    ...
                    print(f"✅ WARP active: {current_ip}")
            else:
                print("⚠️  WARP rotation failed, using direct connection")
        
        async with async_playwright() as p:
            # Use Chrome in HEADED mode with Xvfb (virtual display)
            # Xvfb allows headed mode on VPS/Railway without physical display
            browser = await p.chromium.launch(
                headless=False,  # Run in headed mode (bypasses Turnstile detection)
            ...
            # Create mailbox in SEPARATE tab (context creates new page)
            ...
            try:
                ...
                session_dir = await register_cli_session(context, railway_page, SESSIONS_DIR)
                ...
                # Don't wait for Enter in headless mode
                # input("\n⏸️  Press Enter to close browser...")
                print("\n✅ Account created successfully, closing browser...")
