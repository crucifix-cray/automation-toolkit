...
BRD_WSS = os.environ.get("BRD_WSS") or os.environ.get("ZENROWS_WSS") or ZENROWS_WSS_POOL[0]
...
class TempTfInbox:
    ...
    def _get(self, path, data=None):
        ...
        _old_https = _hc.HTTPSConnection._connection_class if hasattr(_hc.HTTPSConnection, '_connection_class') else None
...
async def run(use_warp=False, cloud_mode=False):
    ...
    if cloud_mode:
        ...
        if is_one_to_one:
            # 1:1 mode - use only the passed one, no locking needed (keep full WSS with apikey for ZenRows)
        ...
        if not is_one_to_one:
          ...
          base_wss = pool_pick
          # ZenRows does not use sessionId, BD does
          if "zenrows.com" in base_wss:
            BRD_WSS = base_wss
            print("☁️  Cloud mode: ZenRows Browser API (ASN rotation), no local WARP")
            print(f"☁️  ZenRows session: {BRD_WSS[:80]}*** (pool {idx % len(BRD_WSS_POOL) + 1}/{len(BRD_WSS_POOL)} ASN rotation)")
          else:
            sep = "&" if "?" in base_wss else "?"
            BRD_WSS = base_wss + f"{sep}sessionId={_uuid.uuid4()}"
            print("☁️  Cloud mode: BD Browser API (ASN rotation), no local WARP")
            print(f"☁️  Fresh BD session: {BRD_WSS[:80]}*** (pool {idx % len(BRD_WSS_POOL) + 1}/{len(BRD_WSS_POOL)} ASN rotation)")
        else:
          # 1:1 mode - just add sessionId for BD, not for ZenRows
          if "zenrows.com" in passed_base:
            BRD_WSS = passed_base
            print("☁️  Cloud mode: ZenRows Browser API (ASN rotation), no local WARP")
            print(f"☁️  ZenRows dedicated session: {BRD_WSS[:80]}***")
          else:
            sep = "&" if "?" in passed_base else "?"
            BRD_WSS = passed_base + f"{sep}sessionId={_uuid.uuid4()}"
            print("☁️  Cloud mode: BD Browser API (ASN rotation), no local WARP")
